...
import { HealthAnalysisData, ComponentProps } from "../types";
import { InputGroup, Form } from "react-bootstrap";
import { FieldValues, UseFormRegister } from "react-hook-form";
import { MdClear } from "react-icons/md";
import { infoFields } from "@/utils";
      ...
      {infoFields.map((field) => (
        <div key={field.name} className="mb-4">
          <label className="flex ml-1 sm:w-[65vw] lg:w-[60vw] mb-[-0.5rem]">
            ...
            {errors[field.name] && (
              <h6 className="text-red-500 ml-3">{`${errors[field.name]?.message}`}</h6>
            )}
          </label>
          ...
          <div className="lg:w-[60vw] sm:w-[65vw] flex flex-col">
            <InputGroup className="flex border-2 border-black rounded-md h-full hover:ring-2 ring-blue-400">
              <Form.Control
                ...
                type="text"
                {...register(`${field.name}`, {
                  required: `${field.asterisk && (formData[field.name] === "" ) ? "Required" : ""}`,
                  ...
                  validate: (value) => {
                    if (field.pattern) {
                      return field.pattern.value.test(value as any) || field.pattern.message;
                  }}
                })}
